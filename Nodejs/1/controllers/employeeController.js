...
const employeeControll = {
    ...
    createEmployee: (request, response) => {
        ...
        new Employee(request.body).save({}, (error, employee) => {
            if (error) {
                ...
                if (employeeValidation(error)) return response.status(400).send(employeeValidation(error));
            ...
            if (employee.managerStatus === true) {
                Employee.updateMany({companyID: employee.companyID, _id: {$ne: employee._id}}, {$set: {managerStatus: false}}, (error, employee) => {
                    ...
                    return response.status(200).send('created and manager changed');
                });
            } else return response.status(201).send('created');
        });
    },
    ...
    updateEmployee: (request, response) => {
        ...
        Employee.findByIdAndUpdate(employeeID, request.body, {runValidators: true}, (error, employee) => {
            if (error) {
                // bad request: employee properties validation
                if (employeeValidation(error)) return response.status(400).send(employeeValidation(error));
            ...
            if(employee.managerStatus === true) {
                Employee.updateMany({companyID: employee.companyID, _id: {$ne: employee._id}}, {$set: {managerStatus: false}}, (error, employee) => {
                    ...
                    return response.status(200).send('updated and manager changed');
                });
            } else return response.status(200).send('updated');
        });
    },
    ...
    mokhaberatEmployees: (request, response) => {
        Employee.find({companyID: '60570f34d1853414508ac336'}).populate('companyID', {companyName: 1, _id: 0}).exec((error, companies) => {
    ...
    companiesManagers: (request, response) => {
        Employee.find({managerStatus: true}).populate('companyID', {companyName: 1, _id: 0}).exec((error, companies) => {
            if (error) return console.log(error);
    ...
    mokhaberatManager: (request, response) => {
        Employee.find({companyID: '60570f34d1853414508ac336', managerStatus: true}, {firstname: 1, lastname: 1, _id: 0}, (error, employees) => {
            if (error) return console.log(error);

            return response.send(employees);
        });
    },
    companiesNameManagerName: (request, response) => {
        Employee.find({managerStatus: true} ,{firstname: 1, lastname: 1, _id: 0}).populate('companyID',{companyName: 1, _id: 0}).exec((error, companies) => {
            if (error) return console.log(error);
